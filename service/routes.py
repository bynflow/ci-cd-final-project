...
def create_counters(name):
    ...
    if name in COUNTER:
        error_message = f"Counter {name} already exists"
        return abort(status.HTTP_409_CONFLICT, error_message)
...
def read_counters(name):
    ...
    if name not in COUNTER:
        error_message = f"Counter {name} does not exist"
        return abort(status.HTTP_404_NOT_FOUND, error_message)
